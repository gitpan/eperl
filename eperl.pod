...
and creating a symlink to the F<eperl> program:
  ...
  $ ln -s @prefix@/bin/eperl /path/to/apache/cgi-bin/nph-eperl
...
Here are some possible variations you could choose:

  "<?" and "!>"
  default ePerl CGI mode delimiters

  "<:" and ":>"
  default ePerl stand-alone filtering mode delimiters

  "<script language='ePerl'>" and "</script>" 
  standard HTML scripting language style

  "<!--#eperl code='" and "' -->"
  NeoScript style

  "<?" and ">"
  PHP/FI style (no longer recommended because can lead 
  to parsing problems. Should be used only for backward 
  compatibility to old ePerl versions 1.x)

=item B<-e> I<end_delimiter>

Sets the Perl block end delimiter string. See option B<-b>
...
When you are installing ePerl as a NPH-CGI/1.1 program (F<nph-eperl>) with
setuid bit enabled (C<chmod u+s>!) then ePerl can switch to the uid/gid of the
B<script owner>. Although this is a very useful feature for script programmers
it can be to risky for you when you are paranoid about security of setuid
programs. If so just don't install ePerl with enabled setuid bit! This is the
reason why ePerl is not installed per default as F<nph-eperl> for your
webserver. For those who decided that this feature is essential for them ePerl
tries hard to make it secure. The following steps have to be successfully
passed before ePerl actually switches its uid/gid (in this order):

  1. The script has to match the following extensions:
     .html, .phtml, .ephtml, .epl, .pl
...
B<IF ONLY ONE OF THOSE STEPS FAIL, NO UID/GID SWITCHING TAKES PLACE!>
You can only disable some steps at compile time. See F<eperl_global.h>.
