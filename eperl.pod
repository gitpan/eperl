...
pages, there is common problem in conjunction with HTML editors.  They cannot
know ePerl blocks, so when you enter those blocks inside the editors they
usually encode some characters with the corresponding HTML entities. The
...
blocks (or the transformation into a CGI script was useless).
...
Specify a directory which is both used for C<#include> directives of the ePerl
preprocessor and added to C<@INC> under runtime.  This option can occur more
than once.

=item B<-P>

This enables the special ePerl Preprocessor which provides the following
CPP-style constructs:
...
The first directive is an include directive which can be used for any stuff,
but actually designed to be used to include other ePerl source files. The
C</absolute/path> is directly accessed on the filesystem, while
C<relative/path> is first searched in the current working directory and then
in all directories specified via ePerl option B<-I>. In the third case the
file is given as an URL, so a HTTP/1.0 request used to retrieve the file.

The other directives implement CPP-style C<#if-[#else-]#endif> constructs, but
with a Perl semantic. While C<#include> really is a preprocessor command which
is evaluated at the preprocessing step, this if-construct is actually just
transformed into a low-level ePerl construct, so are B<not> actually evaluated
...
(where BD is the currently used begin delimiter and ED the end delimiter).
